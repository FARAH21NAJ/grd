...
function deleteRowsWithOrangeColor($day, $timeSlot) {
    global $db1;
    $sql = "DELETE FROM app_book4 WHERE day = $day AND time_slot = '$timeSlot' AND color = 'orange'";
...
function updateOrInsertRecord($selectedDay, $selectedTime, $paymentStatus, $color,$userName) {
   ...
   $sql = "INSERT INTO app_book4 (day, time_slot, payment_status, color, date, patient_name) 
...
function cancelAppointmentsForCurrentMonth() {
    ...
    if (date('j') == 1) {
        ...
        $sql = "DELETE FROM app_book4 WHERE MONTH(CURDATE()) = $currentMonth AND YEAR(CURDATE()) = $currentYear";
...
function cancelAppointmentsForCurrentWeek() {
    ...
    if (date('N') == 1) { // Assuming Monday is the first day of the week
        ...
        $sql = "DELETE FROM app_book4 WHERE WEEK(CURDATE()) = $currentWeek";
